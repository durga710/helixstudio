...
/**
 * GitHub adapter (raw fetch, no Octokit). Every call runs as the signed-in
 * user via withGitAuth() — there is NO workspace/env token fallback; no
 * token means the UI prompts to connect GitHub.
 */

import { activeAuth, type GitProvider } from "./types";

const GH_API = "https://api.github.com";

function activeToken(): string | undefined {
  const auth = activeAuth();
  return auth?.provider === "github" ? auth.token : undefined;
}
