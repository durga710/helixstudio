 ...
 * `seats` seats). The member cap below is enforced server-side in the join
 * route and NEVER depends on Stripe being configured — billingEnabled() only
 * controls whether the upgrade UI is offered.
 *
 * Lapse behavior is deliberately non-destructive: an expired subscription
 * reverts the Space to free caps, which blocks NEW joins when over the limit
 * but never removes members or data.
 */
...
/** Per-seat Space subscription price. */
export function priceIdForSpace(): string {
